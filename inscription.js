...
function validerDateOfBirth() {
  ...
  if (elementvalue >= dateAujourdhui)
    dateError.innerHTML =
      "<span style='color :red'>please select a date that is before today</span>";
  else dateError.innerHTML = "<span style='color :green'>Correct</span>";
}
...
function validateName() {
  ...
  if (!NameValue.match(pattern)) {
    nameError.innerHTML =
      "<span style='color :red'> nom ne doit contenir que des lettres et doit avoir au minimum 1 lettre </span>";
  } else {
...
function validateEmail(email) {
  var regex = /^[a-zA-Z0-9._-]+@esprit.tn$/;
...
emailInput.addEventListener("keyup", function () {
  ...
  if (validateEmail(email)) {
    ...
    emailError.innerHTML = "<span style='color: red'>Email is invalid</span>";
  }
});
if (validateEmail(email)) {
  validationMessage.textContent = "Email is valid";
...
function validatetel() {
  var telvalue = elementdate.value;
  var pattern = /^[0-9]{8}$/;
  if (!telvalue.match(pattern)) {
    telError.innerHTML =
      "<span style='color: red'>Numero de telephone doit contenir exactement 8 chiffres</span>";
  } else {
